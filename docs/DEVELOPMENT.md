...
The desktop app is still run by launching Electron with the local backend process.
...
```powershell
cd ..\backend
pyinstaller src/open_vfd_simulator_backend/main.py --onefile
cd ..
mkdir .\frontend\backend
cp backend/dist/main.exe frontend/backend/main.exe
```

Expected output:

- `frontend\backend\main.exe`
...
- `frontend\dist\OpenVFD Setup 0.1.0.exe`
